...
func TestUserBannerGetBadReq(t *testing.T){
	...
	req = httptest.NewRequest(http.MethodGet, "/user_banner?tag_id=true&feature_id=2", nil)
	req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
	rec = httptest.NewRecorder()
	c = e.NewContext(req, rec)
	err = wrapper.GetUserBanner(c)
	...
	rec = httptest.NewRecorder()
	c = e.NewContext(req, rec)
	assert.EqualError(t, wrapper.GetUserBanner(c), "code=400, message=Invalid format for parameter feature_id: error binding string parameter: strconv.ParseInt: parsing \"true\": invalid syntax")
